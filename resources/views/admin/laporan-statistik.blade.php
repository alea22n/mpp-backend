...
@push('styles')
    <style>
        ...
        .pagination-wrapper-grid {
            display: flex;
            /* Menggantikan .pagination-footer lama */
            justify-content: space-between;
            align-items: center;
            padding: 15px 0;
            font-size: 14px;
        ...
        .pagination-nav .page-item span {
            ...
            padding: 8px 14px;
            border-radius: 12px;
            /* Sudut sangat membulat, sesuai Gambar 1 */
            cursor: pointer;
            font-size: 14px;
            min-width: 40px;
            text-align: center;
        ...
        .pagination-nav .page-item.active span {
            ...
            /* Shadow biru yang lebih menonjol */
        }
...
@section('content')
    <div class="container-fluid">
        ...
        <p style="font-size: 14px; color: var(--muted); margin-bottom: 25px; margin-top:-10px;">
            DPMPTSP - Periode Aktif: <strong>{{ \Carbon\Carbon::parse($dateFrom)->format('d M Y') }} -
                {{ \Carbon\Carbon::parse($dateTo)->format('d M Y') }}</strong>
        </p>
        ...
        <div class="grid">
            <div class="card small">
                <div>
                    ...
                    <div class="num">{{ number_format($stats['total_instansi'], 0, ',', '.') }}</div>
                </div>
            ...
            <div class="card small">
                <div>
                    <h3>Jenis Layanan</h3>
                    <div class="num">{{ number_format($stats['total_layanan'], 0, ',', '.') }}</div>
                </div>
            ...
            <div class="card small">
                <div>
                    ...
                    <div class="num">{{ number_format($stats['total_permohonan'], 0, ',', '.') }}</div>
                </div>
            ...
            <div class="card small">
                <div>
                    ...
                    <div class="num">{{ number_format($stats['pengguna_aktif'], 0, ',', '.') }}</div>
                </div>
        ...
        <div class="list-card">
            ...
            <div style="overflow-x: auto;">
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse($reports as $index => $item)
                            <tr>
                                <td style="text-align: center;">
                                    {{ method_exists($reports, 'firstItem') ? ($reports->firstItem() + $index) : ($index + 1) }}
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" style="text-align: center; padding: 40px; color: #999;">Data tidak ditemukan
            ...
            <div class="pagination-wrapper-grid">
                ...
                <div class="pagination-nav">
                    <div class="pagination" id="statistikPagination"></div>
                </div>
            </div>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        // Pagination Statistik (mirip kelola-instansi)
        const statistikCurrentPage = {{ $reports->currentPage() }};
        const statistikLastPage = {{ $reports->lastPage() }};
        document.addEventListener("DOMContentLoaded", function () {
            // Render pagination custom
            function renderStatistikPagination() {
                const container = document.getElementById('statistikPagination');
                if (!container) return;
                container.innerHTML = "";
                if (statistikLastPage <= 1) return;

                // Prev
                const prevBtn = document.createElement('button');
                prevBtn.innerHTML = '<i class="fa-solid fa-chevron-left"></i>';
                prevBtn.disabled = statistikCurrentPage === 1;
                prevBtn.onclick = () => gotoStatistikPage(statistikCurrentPage - 1);
                container.appendChild(prevBtn);

                // Nomor Halaman
                for (let i = 1; i <= statistikLastPage; i++) {
                    if (i === 1 || i === statistikLastPage || (i >= statistikCurrentPage - 1 && i <= statistikCurrentPage + 1)) {
                        const btn = document.createElement('button');
                        btn.innerText = i;
                        btn.className = i === statistikCurrentPage ? 'active' : '';
                        btn.onclick = () => gotoStatistikPage(i);
                ...
                nextBtn.disabled = statistikCurrentPage === statistikLastPage;
                nextBtn.onclick = () => gotoStatistikPage(statistikCurrentPage + 1);
                container.appendChild(nextBtn);
            }
            function gotoStatistikPage(page) {
                const params = new URLSearchParams(window.location.search);
                params.set('page', page);
                window.location.search = params.toString();
            }
            renderStatistikPagination();
